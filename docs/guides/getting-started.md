...
Noveles uses `flutter_dotenv` for configuration. Create a `.env` file in the project root:

```bash
cp .env.example .env
```

> **Note**: `.env.example` does not exist yet. Create `.env` manually with the following content:
...
The `.env` file is listed in `pubspec.yaml` assets (required for `flutter_dotenv` loading) and should be in `.gitignore`. **Never commit your `.env` file** with real credentials.
...
This runs all migrations and sets up the database schema (tables, RLS policies, storage buckets).

For a remote Supabase project, migrations are applied via the dashboard or CI/CD.
...
Test structure mirrors the lib/ folder: `test/bloc/`, `test/entities/`, `test/repositories/`, `test/use_cases/`, `test/widgets/`.
...
| Emulator not found | Open Android Studio → Device Manager → start an emulator |
