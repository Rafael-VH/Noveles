...
> Complete catalog of all 9 domain entities with field definitions and relationships.
...
| `BookWithRelations` | `lib/shared/domain/entities/book_with_relations.dart` | 25 (+3 lists) | shared |
| `ChapterEntity` | `lib/features/chapters/domain/chapter_entity.dart` | 7 | chapters |
| `ChapterRef` | `lib/features/chapters/domain/chapter_ref.dart` | 5 | chapters |
...
| `UserEntity` | `lib/features/profiles/domain/user_entity.dart` | 6 (+4 getters) | profiles |
| `FavoriteEntity` | `lib/features/favorites/domain/favorite_entity.dart` | 3 | favorites |
...
The core book entity — contains only scalar fields and relation IDs. No imports from other features.
...
Book entity with fully hydrated relation objects. Used when Supabase returns joined data (genres, labels, tooks). This is the type the UI works with for rendering.
...
Lightweight reference for chapter events. Carries only the fields needed to initiate content loading, avoiding the cost of passing full `ChapterEntity` objects through the event bus.
...
**Factory**: `ChapterRef.fromEntity(ChapterEntity entity)` — creates a ref from a full entity.
...
A "took" represents a volume or tome within a book. Books contain multiple tooks, and each took contains multiple chapters.
...
`UserRole.fromString()` parses DB string values. Unknown or null values default to `UserRole.user`.
...
UserEntity ──(createdBy)──→ BookEntity
                              ...
                              │                       │
                              │                       └──(listChapterIds)──→ ChapterEntity ──(tookId)──→ TookEntity
                              │
                              ├──(listGenreIds)──→ GenreEntity (many-to-many via books_genres)
                              │
                              └──(listLabelIds)──→ LabelEntity (many-to-many via books_labels)
