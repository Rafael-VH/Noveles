...
> Supabase Auth integration — login, register, logout, and session state
management.

## Overview

The Auth feature handles authentication via Supabase Auth. It manages the full
session lifecycle: email/password login, registration (which auto-creates a
profile row in `profiles`), logout, and real-time auth state listening. The
`AuthBloc` is a root-level provider that drives role-based routing in `App`.

## Use Cases

| Use Case | Signature | Purpose |
| ---------- | ----------- | --------- |
| Login | `FR<UserEntity>` call(email,pwd) | Sign in with email/password |
| Register | `FR<UserEntity>` call(email,pwd) | Create account + auto-profile |
...
The BLoC also listens to `ListenAuthState` stream and auto-dispatches
`LogoutRequested` on external sign-out events (e.g., token expiration).
...
- [Entities](../../domain/entities.md) — `UserEntity` fields (from profiles
feature)
- [Use Cases](../../domain/use-cases.md) — Auth use case signatures
- [User Types](../../user-types/regular-user.md) — Role-based access
- [Error Handling](../../architecture/error-handling.md) — `AuthFailure`,
`ProfileFailure`
