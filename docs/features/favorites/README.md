...
The Favorites feature allows authenticated users to manage their personal book
favorites. Users can add/remove books from their favorites list and view all
favorited books. The feature uses the `user_favorites` Supabase table with
...
| getFavorites | `FR<L<FE>>` call(String uid) | Get all favorites for user |
| isFavorite | `FR<bool>` call(uid,bid) | Check if book is favorited |

**Implementation**: `lib/features/favorites/data/favorite_repository_impl.dart`
— uses Supabase client to query `user_favorites` table.
...
- [Error Handling](../../architecture/error-handling.md) — `FavoriteFailure`
type
